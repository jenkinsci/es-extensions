...
const ExtensionsContainer: React.SFC<Props> = (props) => <div className={`extension_container ${props.containerClassName || ''}`.trim()}>{props.children}</div>
...
export class ExtensionPoint extends React.Component<Props, State> {
    ...
    render() {
        ...
        return <ExtensionsContainer {...this.props}>
           {exts.map((extension: any, index: number) => { 
                return <Container key={`${this.props.keyPrefix || 'key-'}${index}`} extension={extension} context={this.props.context}/>;
            })}
        </ExtensionsContainer>  
    }
}